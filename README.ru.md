...
возможность, чем путь с доменом. Точный вызов `certbot` для неё в этом скрипте — лучшая попытка, а
не гарантия: если он не совпадёт с флагами вашей версии certbot, скрипт заметит ошибку и
откатится на самоподписанный сертификат, а не оставит установку в полусломанном состоянии. Если
есть возможность направить домен на сервер — этот путь гораздо более проверенный.
